console.info("%c 🌦️ DOMHOUSE-WEATHER-CARD v7.9 IS LOADED ", "color: white; background: #2196f3; font-weight: bold;");
...
class DomHouseWeatherCard extends LitElement {
  ...
  _getGaugeConfig(entity, suffix) {
    ...
    let calculatedIcon = auto.icon;
    let calculatedColor = auto.color;
    ...
    if (entity) {
        ...
        if (!isNaN(val) && entity.attributes.unit_of_measurement) {
            ...
            calculatedIcon = conf[`icon_on_${suffix}`] || auto.icon;
            calculatedColor = conf[`color_${suffix}`] || auto.color;
            ...
            const userIconOn = conf[`icon_on_${suffix}`];
            const userIconOff = conf[`icon_off_${suffix}`];

            if (isActive) {
                calculatedIcon = userIconOn || auto.icon;
            } else {
                calculatedIcon = userIconOff || userIconOn || auto.icon;
            }

            const defaultColorOn = "#4caf50";
            const defaultColorOff = "#e53935";
            const userColorOn = conf[`color_on_${suffix}`];
            const userColorOff = conf[`color_off_${suffix}`];
            const staticColor = conf[`color_${suffix}`];

            if (isActive) {
                calculatedColor = userColorOn || staticColor || defaultColorOn;
            } else {
                calculatedColor = userColorOff || staticColor || defaultColorOff;
            }
        }
    }
    ...
    if (badgeEntityId && this.hass.states[badgeEntityId]) {
        ...
        const defaultIcon = conf[`badge_icon_${suffix}`] || bStateObj.attributes.icon || "mdi:circle";
        const iconOn = conf[`badge_icon_on_${suffix}`] || defaultIcon;
        const iconOff = conf[`badge_icon_off_${suffix}`] || defaultIcon;
  ...
  renderGauge(entity, conf) {
    ...
    if (conf.isNumeric) {
        ...
        const translations = {
            ...
            "on": isOpening ? "Aperta" : "Acceso",
            "off": isOpening ? "Chiusa" : "Spento",
...
// =============================================================================
//  VISUAL EDITOR CLASS (v7.4 - Layout Fixes)
// =============================================================================
    ...
    const renderSensorOptions = (label, suffix) => html`
      <div class="sensor-group">
          ...
          <div class="sensor-options">
           ...
           <div class="sensor-options">
              <ha-textfield
                  .value=${this._config[`color_on_${suffix}`] || ''}
                  .configValue=${`color_on_${suffix}`}
                  .label=${"Colore ON"}
                  placeholder="#4caf50"
                  @input=${this._valueChanged}
              ></ha-textfield>
               <ha-textfield
                  .value=${this._config[`color_off_${suffix}`] || ''}
                  .configValue=${`color_off_${suffix}`}
                  .label=${"Colore OFF"}
                  placeholder="#e53935"
                  @input=${this._valueChanged}
              ></ha-textfield>
...
// --- REGISTRAZIONE NELLA PICKER LIST (MENU AGGIUNGI SCHEDA) ---
window.customCards = window.customCards || [];
window.customCards.push({
  type: "domhouse-weather-card",
  name: "DomHouse Weather Card",
  description: "Una card All-in-One per Meteo e Sensori con Badge e Azioni.",
  preview: true,
  documentationURL: "https://github.com/SalvatoreITA/domhouse-weather-card",
});
