  ...
  _getGaugeConfig(entity, suffix) {
    ...
    if (entity) {
        ...
        if (!isNaN(val) && entity.attributes.unit_of_measurement) {
            // --- NUMERICO (Sensore classico) ---
            auto.isNumeric = true;
            const u = entity.attributes.unit_of_measurement;
            auto.unit = u;

            // Auto-detect colore e icona base
            ...
            // Icona e Colore Statici (override manuale)
            calculatedIcon = conf[`icon_on_${suffix}`] || auto.icon; // Uso campo ON come default se statico
            ...
            // Icone Default in base al tipo entità
            ...
            const defaultColorOn = "#4caf50"; // Verde
            const defaultColorOff = "#e53935"; // Rosso
            const userColorOn = conf[`color_on_${suffix}`];
            const userColorOff = conf[`color_off_${suffix}`];
            const staticColor = conf[`color_${suffix}`]; // Campo colore generico
    ...
    if (badgeEntityId && this.hass.states[badgeEntityId]) {
        ...
        // Determinazione Icona Badge ON vs OFF
...
// =============================================================================
//  VISUAL EDITOR CLASS (v7.3 - Dynamic Main Colors)
// =============================================================================
